  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:title" content={`${user?.firstName}'s Profile`} />
          ...
          <Card className="backdrop-blur-md bg-white/80 dark:bg-[#1f2937]/80 border shadow-lg rounded-3xl p-6 md:p-10 flex flex-col md:flex-row gap-10">
            <div className="flex flex-col items-center md:w-[380px] gap-4">
              <Avatar className="w-44 h-44 ring-4 ring-blue-400 dark:ring-green-500 shadow-lg relative">
                <AvatarImage src={typeof input.file === 'object' ? URL.createObjectURL(input.file) : (user?.photoUrl || userLogo)} />
              ...
              <div className="flex gap-5 mt-2 text-2xl">
                {[['facebook', FaFacebook, 'text-blue-600'], ['linkedin', FaLinkedin, 'text-blue-500'], ['github', FaGithub, 'text-black dark:text-gray-200'], ['instagram', FaInstagram, 'text-pink-500']].map(([key, Icon, color]) => (
                  <Link key={key} to={user?.[key] || '#'} target="_blank">
                    <Icon className={user?.[key] ? `${color} hover:brightness-125 transition` : 'text-gray-400 cursor-not-allowed'} />
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="grid gap-4 py-4">
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <Input name="firstName" aria-label="First Name" value={input.firstName} onChange={changeEventHandler} />
                  </div>
                  <div className="flex-1">
                    <Label>Last Name</Label>
                    <Input name="lastName" aria-label="Last Name" value={input.lastName} onChange={changeEventHandler} />
                  </div>
                </div>
                <div className="flex gap-2">
                  <div className="flex-1">
                ...
                <div>
                  ...
                  {input.file && typeof input.file === 'object' && (
                    <img
                      ...
                      alt="preview"
                      className="h-24 w-24 mt-2 object-cover rounded-full ring ring-blue-500"
                    />
                  )}
                </div>
              </div>
